...
export function createOverlays(root,onStatus=()=>{}) {
 let sequence=0,generation=0,previous=null;const active=[],lists=new Map()
 ...
 return {
  async play(settings){
   const current=generation;let url;try{url=await source(settings.triggerVideoUrl)}catch{onStatus('트리거영상 목록을 확인하세요.');return}if(current!==generation)return
   while(active.length>=settings.maxOverlays)remove(active[0])
   ...
   video.play().catch(()=>{if(!active.includes(item))return;onStatus('트리거영상 재생 실패');remove(item)})
  },
  ...
  clear(){generation++;for(const item of [...active])remove(item)},
