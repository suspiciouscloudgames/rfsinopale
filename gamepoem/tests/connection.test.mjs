...
const source=readFileSync(new URL('../site/connection.js',import.meta.url),'utf8').replace('export function','function')+'\nglobalThis.connectScreen=connectScreen'
function fixture(display=false,legacy=false){
  ...
  const context=vm.createContext({Date:{now:()=>now},crypto:legacy?{}:{randomUUID:()=> 'local'},Peer,window:{Peer,addEventListener(){},removeEventListener(){}},document:{hidden:false,documentElement:{dataset:{}},addEventListener(){},removeEventListener(){}},setTimeout(fn,delay){const t={fn,at:now+delay};timers.push(t);return t},clearTimeout(t){if(t)t.done=true},setInterval(fn,delay){const t={fn,at:now+delay,delay};timers.push(t);return t},clearInterval(t){if(t)t.done=true}})
